...
func AddUserController(c echo.Context) error {
	...
	if errInsert != nil {
		return c.JSON(http.StatusInternalServerError, helper.FailedResponse("failed insert data"))
	}
	return c.JSON(http.StatusCreated, helper.SuccessResponse("success create user"))
}
